...
[github-badge]: https://img.shields.io/badge/github-chksum--rs%2Fcore-24292e?style=flat-square&logo=github
[github]: https://github.com/chksum-rs/core
...
Directory-digest semantics, framing, and collision classes: see
[Directory digests](https://github.com/chksum-rs/core/blob/master/docs/DIRECTORY-DIGESTS.md).
...
* `nonblocking-open`: On Unix, opens files with `O_NONBLOCK` so a race-swapped FIFO or slow device cannot hang
  traversal; off by default. See [Hardening](https://github.com/chksum-rs/core/blob/master/docs/HARDENING.md).
...
* **Symlinks are followed**: linked files are hashed and linked directories are traversed.
  See [Symlinks](https://github.com/chksum-rs/core/blob/master/docs/SYMLINKS.md).
...
* **Names and structure are opt-in**: the default `NameMode::Off` hashes file contents only (distinct trees can
  ...
  See [Directory digests](https://github.com/chksum-rs/core/blob/master/docs/DIRECTORY-DIGESTS.md).
* **Irregular files abort by default**: an entry that is neither a regular file nor a directory aborts traversal
  ...
  See [Hardening](https://github.com/chksum-rs/core/blob/master/docs/HARDENING.md).
* **Traversal limits exist**: `Error::TraversalTooDeep` past `max_directory_depth` (default 64) and
  `Error::DirectoryTooLarge` past `max_directory_entries` (default 10,000,000).
  See [Hardening](https://github.com/chksum-rs/core/blob/master/docs/HARDENING.md).

## Documentation

* [Symlinks](https://github.com/chksum-rs/core/blob/master/docs/SYMLINKS.md) — following, cycle/fan-out bounding,
  platform coverage, and `ELOOP` interaction.
* [Directory digests](https://github.com/chksum-rs/core/blob/master/docs/DIRECTORY-DIGESTS.md) — digest semantics,
  `NameMode::FileName` framing, and reproducibility.
* [Hardening](https://github.com/chksum-rs/core/blob/master/docs/HARDENING.md) — `nonblocking-open`, race windows,
  irregular-file policy, and traversal limits.
* [Gotchas](https://github.com/chksum-rs/core/blob/master/docs/GOTCHAS.md) — call shapes that compile and run but
